<?php

namespace Database\Seeders;

use Illuminate\Database\Seeder;
use Illuminate\Database\Console\Seeds\WithoutModelEvents;

use Illuminate\Support\Facades\DB;

class CuadernoSeeder extends Seeder
{
    public function run(): void
    {
        $estanciaId = DB::table('estancias')->value('id_estancia');

        DB::table('cuadernos_practicas')->insert([
            'archivo' => null,
            'archivo_vacio' => 'plantilla_cuaderno.pdf',
            'id_estancia' => $estanciaId,
            'created_at' => now(),
            'updated_at' => now(),
        ]);

        $cuadernoId = DB::table('cuadernos_practicas')->where('id_estancia', $estanciaId)->value('id_cuaderno');

        DB::table('entregas')->insert([
            [
                'archivo' => 'entrega_1.pdf',
                'fecha' => now()->subDays(7)->toDateString(),
                'id_cuaderno' => $cuadernoId,
                'created_at' => now(),
                'updated_at' => now(),
            ],
            [
                'archivo' => 'entrega_2.pdf',
                'fecha' => now()->toDateString(),
                'id_cuaderno' => $cuadernoId,
                'created_at' => now(),
                'updated_at' => now(),
            ],
            [
                'archivo' => 'entrega_3.pdf',
                'fecha' => now()->addDays(14)->toDateString(),
                'id_cuaderno' => $cuadernoId,
                'created_at' => now(),
                'updated_at' => now(),
            ],
        ]);

        DB::table('notas_cuaderno')->insert([
            'nota' => 8.50,
            'id_cuaderno' => $cuadernoId,
            'created_at' => now(),
            'updated_at' => now(),
        ]);
    }
}